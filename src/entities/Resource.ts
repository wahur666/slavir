...
export default class Resource extends Phaser.GameObjects.Sprite {
    ...
    constructor(scene: GameScene, x: number, y: number, texture: string, gameTile: GameTile, priority: number, free: () => void) {
    ...
    update() {
        if (this._occupied) {
            this.availableResource -= Date.now() - this.lastHarvestUpdate;
        }
